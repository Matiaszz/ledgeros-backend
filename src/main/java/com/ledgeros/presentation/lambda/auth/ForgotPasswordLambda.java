...
public class ForgotPasswordLambda implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent input, Context context) {
        return LambdaWrapper.execute(() -> {
            ...
            try {
                request = OBJECT_MAPPER.readValue(input.getBody(), ForgotPasswordRequest.class);
            } catch (Exception e) {
                throw new LambdaException("Failed to parse forgot password request body: " + e.getMessage(), INVALID_REQUEST);
            }
